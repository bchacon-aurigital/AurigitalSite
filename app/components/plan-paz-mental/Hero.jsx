"use client";
import { ArrowDownRight } from "lucide-react";
import { useLanguage } from "../../context/LanguageContext";
import { useContactModal } from '../../context/ContactModalContext';

const Hero = () => {
  const { translations } = useLanguage();
  const { openModal } = useContactModal();
  const planData = translations.planPazMental.hero;

  const renderTextWithSpans = (text) => {
    return text.split("<span>").map((part, index) => {
      if (part.includes("</span>")) {
        const [spanText, remainingText] = part.split("</span>");
        return (
          <span key={index}>
            <span className="text-[#FFFFFF]">{spanText}</span>
            {remainingText}
          </span>
        );
      }
      return part;
    });
  };

  return (
    <section className="max-w-[110rem] mx-auto px-4 sm:px-6 lg:px-0">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div
          className="md:col-span-2 lg:col-span-2 bg-[#B2FF00] rounded-2xl sm:rounded-3xl p-6 sm:p-8 flex flex-col justify-center min-h-[250px] sm:min-h-[300px] lg:min-h-[400px]"
          data-aos="fade-right"
        >
          <div className="flex flex-col gap-3 sm:gap-4">
            <ArrowDownRight className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 xl:w-24 xl:h-24 text-black" />
            <h1 className="text-xl md:text-2xl lg:text-3xl text-[#101010]/60 mb-2 mt-4 sm:mt-6 lg:mt-8 xl:mt-12 font-qurova font-medium">
              {planData.subtitle}
            </h1>

            <h2 className="text-2xl sm:text-3xl md:text-4xl xl:text-5xl font-medium text-black leading-tight font-qurova">
              {planData.title}
            </h2>
          </div>
        </div>

        <div className="md:col-span-2 lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
          <div
            className="sm:col-span-2 bg-[#101010] border border-[#515151] rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 text-white flex h-full items-center"
            data-aos="fade-left"
            data-aos-delay="100"
          >
            <p className="text-sm sm:text-base md:text-lg lg:text-xl text-[#FFFFFF]/60 leading-tight sm:leading-relaxed">
              {renderTextWithSpans(planData.description)}
            </p>
          </div>

          <div
            className="bg-[#101010] border border-[#515151] rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 text-[#E0E0E0] justify-between flex flex-col"
            data-aos="fade-up"
            data-aos-delay="200"
          >
            <h3 className="text-base sm:text-lg lg:text-xl font-medium mb-3 sm:mb-2 font-qurova">
              {planData.evolutionTitle.split(' ').slice(0, 2).join(' ')}<br />
              {planData.evolutionTitle.split(' ').slice(2).join(' ')}
            </h3>
            <button
              onClick={openModal}
              className="bg-[#101010] w-full sm:max-w-[180px] duration-300 group text-white hover:text-[#B2FF00] rounded-full px-6 py-2 border border-[#515151] hover:border-[#B2FF00] transition-colors flex flex-row items-center justify-center whitespace-nowrap"
            >
              <span className="text-sm px-3">
                {planData.cta}
              </span>
              <div className="bg-white rounded-full p-1 group-hover:bg-[#B2FF00] transition-colors w-6 h-6 inline-flex items-center justify-center text-black duration-300 flex-shrink-0">
                <ArrowDownRight className="w-3 h-3" />
              </div>
            </button>
          </div>

          <div
            className="bg-[#101010] border border-[#515151] rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 text-[#E0E0E0] justify-between flex flex-col"
            data-aos="fade-up"
            data-aos-delay="300"
          >
            <h3 className="text-base sm:text-lg lg:text-xl font-medium mb-3 sm:mb-2 font-qurova">
              {planData.focusTitle.split(' ').slice(0, 2).join(' ')}<br />
              {planData.focusTitle.split(' ').slice(2).join(' ')}
            </h3>
            <div className="text-sm text-[#FFFFFF]/60">
              {planData.focusSubtitle}
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Hero;