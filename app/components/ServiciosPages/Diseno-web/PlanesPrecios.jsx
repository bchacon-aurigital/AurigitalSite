...
function PlanCard({ name, price, timeline, idealPara, features, featured, idealParaLabel, buttonText, onButtonClick }) {
  return (
    <div className={`bg-[#1c1c1c] rounded-[26px] flex flex-col items-center overflow-hidden ${featured ? 'px-6 md:px-8 lg:px-10 pt-10 lg:pt-[52px] pb-10 lg:pb-[44px] gap-7' : 'px-5 md:px-7 lg:px-8 pt-8 lg:pt-10 pb-8 lg:pb-10 gap-6'}`}>
      <div className="border-b border-[#3b3b3b] pb-5 w-full flex flex-col gap-4">
        ...
        <p className={`font-space-grotesk font-medium tracking-[-0.36px] leading-[22px] ${featured ? 'text-sm md:text-base' : 'text-xs md:text-sm'}`}>
          <span className="text-white/80">{idealParaLabel}</span>
      ...
      <div className="flex flex-col gap-6 items-center justify-center w-full">
        ...
        <button onClick={onButtonClick} className={`border-[1.5px] border-[#b2ff00] rounded-[12px] w-full font-space-grotesk font-medium text-[#b2ff00] cursor-pointer transition-colors hover:bg-[#b2ff00] hover:text-[#1c1c1c] ${featured ? 'px-5 py-3 text-sm md:text-base' : 'px-4 py-2.5 text-xs md:text-sm'}`}>
          {buttonText}
        </button>
      </div>
    </div>
...
export default function PlanesPrecios({ plans, pazMental, subtitle, sectionTitle, sectionDescription, pricingNote, idealParaLabel, buttonText }) {
  ...
  return (
    <section className="bg-[#e9e9e9] px-6 md:px-10 lg:px-14 py-10 lg:py-16">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {plans.map((plan, i) => (
            <PlanCard key={i} {...plan} featured={i === 1} idealParaLabel={idealParaLabel} buttonText={buttonText} onButtonClick={openModal} />
          ))}
        </div>
